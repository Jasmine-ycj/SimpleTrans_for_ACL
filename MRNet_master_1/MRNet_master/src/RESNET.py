...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1, dim=64):
        self.inplanes = 8
        super(ResNet, self).__init__()

        m = OrderedDict()
        m['conv1'] = nn.Conv2d(1, 8, kernel_size=1, stride=2, padding=0, bias=False)
...
def resnet18(dim):
    model = ResNet(BasicBlock, [2, 2, 2, 2], dim=dim)
    return model


def resnet18small(dim):
    model = ResNet(BasicBlock, [2, 4, 2, 1], dim=dim)
    return model


def resnet18big():
    model = ResNet(BasicBlock, [2, 4, 4, 2], dim=64)
    return model
